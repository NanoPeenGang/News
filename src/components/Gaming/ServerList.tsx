...
export default function ServerList({ servers, activeServerId, onServerSelect }: ServerListProps) {
  return (
    ...
    >
      ...
      <Link
        href="/"
        title="Back to Aegis Monitor"
        className="w-12 h-12 rounded-2xl flex items-center justify-center transition-all duration-200 hover:rounded-xl mb-1"
        style={{ background: 'rgba(6,182,212,0.1)', border: '2px solid rgba(6,182,212,0.3)' }}
      >
        <ArrowLeft size={18} style={{ color: '#06b6d4' }} />
      </Link>
